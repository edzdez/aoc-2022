...
; wish clojure wrapped test/expr pairs in parens like scheme, cuz this #_=> trick for indentation looks pretty horrible
; (though it's better than the alternatives)
; #_=> trick is from [here](https://blog.danieljanus.pl/2020/02/10/cond-indentation/)
(defn correct-order-comp [l r]
  (cond (and (empty? l) (empty? r)) 0
        (empty? l) -1
        (empty? r) 1
        (and (int? (first l)) (int? (first r)) (= (first l) (first r))) (recur (rest l) (rest r))
        (and (int? (first l)) (int? (first r)))
        #_=> (let [res (compare (first l) (first r))]
               (if (= res 0) (recur (rest l) (rest r)) res))
        (and (vector? (first l)) (vector? (first r)))
        #_=> (let [res (correct-order-comp (first l) (first r))]
               (if (= res 0) (recur (rest l) (rest r)) res))
